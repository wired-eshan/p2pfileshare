...
# p2pfileshare VPS Setup Script

# Exit on error
set -e

echo "=== p2pfileshare VPS Setup Script ==="
echo "This script will install Java, Node.js, Nginx, and set up p2pfileshare."
...
# git clone https://github.com/yourusername/p2pfileshare.git
# cd p2pfileshare
...
# Create the p2pfileshare configuration file with the correct content
echo "Creating /etc/nginx/sites-available/p2pfileshare..."
cat <<EOF | sudo tee /etc/nginx/sites-available/p2pfileshare
server {
    listen 80;
...
# Create the symbolic link to enable the p2pfileshare site
sudo ln -sf /etc/nginx/sites-available/p2pfileshare /etc/nginx/sites-enabled/p2pfileshare
...
if [ $? -eq 0 ]; then
    ...
    echo "Nginx configuration test failed. Please check /etc/nginx/nginx.conf and /etc/nginx/sites-available/p2pfileshare."
    exit 1
fi
...
echo "=== Setup Complete ==="
echo "p2pfileshare is now running on your VPS!"
